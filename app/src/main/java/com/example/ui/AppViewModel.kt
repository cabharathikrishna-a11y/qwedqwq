...
class AppViewModel(
    ...
    val nestedTabParents: StateFlow<Map<Screen, Screen>> = _nestedTabParents.asStateFlow()
    ...
    private val _hideStatusBarInFullScreen = MutableStateFlow(true)
    val hideStatusBarInFullScreen: StateFlow<Boolean> = _hideStatusBarInFullScreen.asStateFlow()

    private val _showClockTimeInFullScreen = MutableStateFlow(true)
    val showClockTimeInFullScreen: StateFlow<Boolean> = _showClockTimeInFullScreen.asStateFlow()

    private val _showBatteryInFullScreen = MutableStateFlow(true)
    val showBatteryInFullScreen: StateFlow<Boolean> = _showBatteryInFullScreen.asStateFlow()
    ...
    private val _instagramOverrideOfficialApp = MutableStateFlow(false)
    val instagramOverrideOfficialApp: StateFlow<Boolean> = _instagramOverrideOfficialApp.asStateFlow()

    private val _instagramFilterNotifications = MutableStateFlow(true)
    val instagramFilterNotifications: StateFlow<Boolean> = _instagramFilterNotifications.asStateFlow()

    private val _instagramReelsBlocked = MutableStateFlow(true)
    val instagramReelsBlocked: StateFlow<Boolean> = _instagramReelsBlocked.asStateFlow()

    private val _instagramStoriesBlocked = MutableStateFlow(false)
    val instagramStoriesBlocked: StateFlow<Boolean> = _instagramStoriesBlocked.asStateFlow()

    private val _instagramMessagesBlocked = MutableStateFlow(false)
    val instagramMessagesBlocked: StateFlow<Boolean> = _instagramMessagesBlocked.asStateFlow()

    private val _instagramExploreBlocked = MutableStateFlow(true)
    val instagramExploreBlocked: StateFlow<Boolean> = _instagramExploreBlocked.asStateFlow()

    private val _instagramNotificationsBlocked = MutableStateFlow(false)
    val instagramNotificationsBlocked: StateFlow<Boolean> = _instagramNotificationsBlocked.asStateFlow()

    private val _instagramAllowSharedReels = MutableStateFlow(true)
    val instagramAllowSharedReels: StateFlow<Boolean> = _instagramAllowSharedReels.asStateFlow()
    ...
    fun setInstagramFilterNotifications(enabled: Boolean) {
        _instagramFilterNotifications.value = enabled
        val prefs = getApplication<android.app.Application>().getSharedPreferences("app_prefs", android.content.Context.MODE_PRIVATE)
        prefs.edit().putBoolean("instagram_filter_notifications", enabled).apply()
    }

    fun setInstagramReelsBlocked(blocked: Boolean) {
        _instagramReelsBlocked.value = blocked
        val prefs = getApplication<android.app.Application>().getSharedPreferences("app_prefs", android.content.Context.MODE_PRIVATE)
        prefs.edit().putBoolean("ig_reels_blocked", blocked).apply()
        com.example.util.AppBlockHelper.setInstagramReelsBlocked(getApplication(), blocked)
    }

    fun setInstagramStoriesBlocked(blocked: Boolean) {
        _instagramStoriesBlocked.value = blocked
        val prefs = getApplication<android.app.Application>().getSharedPreferences("app_prefs", android.content.Context.MODE_PRIVATE)
        prefs.edit().putBoolean("ig_stories_blocked", blocked).apply()
        com.example.util.AppBlockHelper.setInstagramStoriesBlocked(getApplication(), blocked)
    }

    fun setInstagramMessagesBlocked(blocked: Boolean) {
        _instagramMessagesBlocked.value = blocked
        val prefs = getApplication<android.app.Application>().getSharedPreferences("app_prefs", android.content.Context.MODE_PRIVATE)
        prefs.edit().putBoolean("ig_messages_blocked", blocked).apply()
    }

    fun setInstagramExploreBlocked(blocked: Boolean) {
        _instagramExploreBlocked.value = blocked
        val prefs = getApplication<android.app.Application>().getSharedPreferences("app_prefs", android.content.Context.MODE_PRIVATE)
        prefs.edit().putBoolean("ig_explore_blocked", blocked).apply()
        com.example.util.AppBlockHelper.setIgExploreBlocked(getApplication(), blocked)
    }

    fun setInstagramNotificationsBlocked(blocked: Boolean) {
        _instagramNotificationsBlocked.value = blocked
        val prefs = getApplication<android.app.Application>().getSharedPreferences("app_prefs", android.content.Context.MODE_PRIVATE)
        prefs.edit().putBoolean("ig_notifications_blocked", blocked).apply()
    }

    fun setInstagramAllowSharedReels(allowed: Boolean) {
        _instagramAllowSharedReels.value = allowed
        val prefs = getApplication<android.app.Application>().getSharedPreferences("app_prefs", android.content.Context.MODE_PRIVATE)
        prefs.edit().putBoolean("ig_allow_shared_reels", allowed).apply()
        com.example.util.AppBlockHelper.setIgAllowSharedReels(getApplication(), allowed)
    }

    // YouTube Web App / AntiTube States
    private val _youtubeWebAppEnabled = MutableStateFlow(false)
    val youtubeWebAppEnabled: StateFlow<Boolean> = _youtubeWebAppEnabled.asStateFlow()

    private val _youtubeOverrideOfficialApp = MutableStateFlow(false)
    val youtubeOverrideOfficialApp: StateFlow<Boolean> = _youtubeOverrideOfficialApp.asStateFlow()

    private val _youtubeShortsBlocked = MutableStateFlow(true)
    val youtubeShortsBlocked: StateFlow<Boolean> = _youtubeShortsBlocked.asStateFlow()

    private val _youtubeAllowSubscribedShorts = MutableStateFlow(true)
    val youtubeAllowSubscribedShorts: StateFlow<Boolean> = _youtubeAllowSubscribedShorts.asStateFlow()

    private val _youtubeBlockHomeFeed = MutableStateFlow(true)
    val youtubeBlockHomeFeed: StateFlow<Boolean> = _youtubeBlockHomeFeed.asStateFlow()

    private val _youtubeSearchBlocked = MutableStateFlow(false)
    val youtubeSearchBlocked: StateFlow<Boolean> = _youtubeSearchBlocked.asStateFlow()
    ...
    fun setYouTubeWebAppEnabled(enabled: Boolean) {
        ...
        val prefs = getApplication<android.app.Application>().getSharedPreferences("app_prefs", android.content.Context.MODE_PRIVATE)
        prefs.edit().putBoolean("youtube_web_app_enabled", enabled).apply()
        com.example.util.AppBlockHelper.setYtWebAppEnabled(getApplication(), enabled)
        if (enabled) {
            com.example.util.ShortcutUtils.createYouTubeShortcut(getApplication())
        }
    }
    ...
    fun setYouTubeShortsBlocked(blocked: Boolean) {
        _youtubeShortsBlocked.value = blocked
        val prefs = getApplication<android.app.Application>().getSharedPreferences("app_prefs", android.content.Context.MODE_PRIVATE)
        prefs.edit().putBoolean("yt_shorts_blocked", blocked).apply()
    ...
    fun setYouTubeAllowSubscribedShorts(allowed: Boolean) {
        ...
        val prefs = getApplication<android.app.Application>().getSharedPreferences("app_prefs", android.content.Context.MODE_PRIVATE)
        prefs.edit().putBoolean("yt_allow_subscribed_shorts", allowed).apply()
        com.example.util.AppBlockHelper.setYtAllowSubscribedShorts(getApplication(), allowed)
    }

    fun setYouTubeBlockHomeFeed(blocked: Boolean) {
        _youtubeBlockHomeFeed.value = blocked
        val prefs = getApplication<android.app.Application>().getSharedPreferences("app_prefs", android.content.Context.MODE_PRIVATE)
        prefs.edit().putBoolean("yt_block_home_feed", blocked).apply()
        com.example.util.AppBlockHelper.setYtBlockHomeFeed(getApplication(), blocked)
    }

    fun setYouTubeSearchBlocked(blocked: Boolean) {
        _youtubeSearchBlocked.value = blocked
        val prefs = getApplication<android.app.Application>().getSharedPreferences("app_prefs", android.content.Context.MODE_PRIVATE)
        prefs.edit().putBoolean("yt_search_blocked", blocked).apply()
        com.example.util.AppBlockHelper.setYtSearchBlocked(getApplication(), blocked)
    }

    fun setYouTubeCommentsBlocked(blocked: Boolean) {
        _youtubeCommentsBlocked.value = blocked
        val prefs = getApplication<android.app.Application>().getSharedPreferences("app_prefs", android.content.Context.MODE_PRIVATE)
        prefs.edit().putBoolean("yt_comments_blocked", blocked).apply()
        com.example.util.AppBlockHelper.setYtCommentsBlocked(getApplication(), blocked)
    }
    ...
    fun updateHideStatusBarInFullScreen(enabled: Boolean) {
        _hideStatusBarInFullScreen.value = enabled
        prefs.edit().putBoolean("hide_status_bar_in_fullscreen", enabled).apply()
    }

    fun updateShowClockTimeInFullScreen(enabled: Boolean) {
        _showClockTimeInFullScreen.value = enabled
        prefs.edit().putBoolean("show_clock_time_in_fullscreen", enabled).apply()
    }

    fun updateShowBatteryInFullScreen(enabled: Boolean) {
        _showBatteryInFullScreen.value = enabled
        prefs.edit().putBoolean("show_battery_in_fullscreen", enabled).apply()
    }
    ...
    fun setTabParent(child: Screen, parent: Screen?) {
        val current = _nestedTabParents.value.toMutableMap()
        ...
        _nestedTabParents.value = current
        val serialized = current.entries.joinToString(",") { "${it.key.name}:${it.value.name}" }
        prefs.edit().putString("nested_tab_parents", serialized).apply()
    }

    fun clearAllTabNesting() {
        _nestedTabParents.value = emptyMap()
        prefs.edit().remove("nested_tab_parents").apply()
    }

    fun loadNestedTabParentsFromPrefs() {
        val raw = prefs.getString("nested_tab_parents", "") ?: ""
        if (raw.isNotBlank()) {
            val map = mutableMapOf<Screen, Screen>()
            raw.split(",").forEach { pair ->
                val parts = pair.split(":")
                if (parts.size == 2) {
                    try {
                        val child = Screen.valueOf(parts[0])
                        val parent = Screen.valueOf(parts[1])
                        if (child != parent) {
                            map[child] = parent
                        }
                    } catch (_: Throwable) {}
    ...
    fun createContact(
        ...
        viewModelScope.launch {
            ...
            if (!_isContactsSyncPaused.value) {
                ...
                // Also trigger background Google Contacts sync if Google Account connected
                try {
                    val app = getApplication<android.app.Application>()
                    syncGoogleContacts(app)
                } catch (e: Exception) {
    ...
    fun deleteContact(contact: Contact) {
        viewModelScope.launch {
            ...
            if (!_isContactsSyncPaused.value) {
                ...
                try {
                    if (!contact.googleContactId.isNullOrEmpty()) {
                        com.example.util.GoogleContactsSyncManager.deleteGoogleContact(getApplication(), contact.googleContactId)
                    }
                } catch (e: Exception) {
                    // Ignore
                }
    ...
    val temporaryMediaIntent = kotlinx.coroutines.flow.MutableStateFlow<TemporaryMediaViewData?>(null)

    fun setTemporaryMediaIntent(data: TemporaryMediaViewData?) {
        temporaryMediaIntent.value = data
    }
    ...
    init {
        ...
        _instagramReelsBlocked.value = prefs.getBoolean("ig_reels_blocked", true)
        _instagramStoriesBlocked.value = prefs.getBoolean("ig_stories_blocked", false)
        _instagramMessagesBlocked.value = prefs.getBoolean("ig_messages_blocked", false)
        _instagramExploreBlocked.value = prefs.getBoolean("ig_explore_blocked", true)
        _instagramNotificationsBlocked.value = prefs.getBoolean("ig_notifications_blocked", false)
        _instagramAllowSharedReels.value = prefs.getBoolean("ig_allow_shared_reels", true)
        _youtubeWebAppEnabled.value = prefs.getBoolean("youtube_web_app_enabled", false)
        _youtubeOverrideOfficialApp.value = prefs.getBoolean("youtube_override_official_app", false)
        _youtubeShortsBlocked.value = prefs.getBoolean("yt_shorts_blocked", true)
        _youtubeAllowSubscribedShorts.value = prefs.getBoolean("yt_allow_subscribed_shorts", true)
        _youtubeBlockHomeFeed.value = prefs.getBoolean("yt_block_home_feed", true)
        _youtubeSearchBlocked.value = prefs.getBoolean("yt_search_blocked", false)
        _youtubeCommentsBlocked.value = prefs.getBoolean("yt_comments_blocked", false)
...
fun createAndPackageSharedTaskFolder(
    ...
    private val _moviesLeaderboard = MutableStateFlow<List<com.example.data.GroupBingerStat>>(emptyList())
    val moviesLeaderboard: StateFlow<List<com.example.data.GroupBingerStat>> = _moviesLeaderboard.asStateFlow()

    private var moviesRtdbListener: com.google.firebase.database.ValueEventListener? = null
    private var leaderboardRtdbListener: com.google.firebase.database.ValueEventListener? = null
    ...
    private fun saveMovieTrackerData(items: List<com.example.data.MovieItem>) {
        try {
            ...
            // Push to Realtime Database for multi-device sync
            val email = if (_userEmail.value.isNotEmpty()) _userEmail.value else com.example.api.DynamicCommandManager.activeEmail
            if (email.isNotEmpty()) {
                val sanitizedEmail = com.example.api.DevicePresenceManager.sanitizeEmail(email)
                val dbUrl = com.example.api.FirebaseConfig.getDatabaseUrl(getApplication())
                if (dbUrl.isNotEmpty()) {
                    val rtdb = com.google.firebase.database.FirebaseDatabase.getInstance(dbUrl)
                    rtdb.getReference("FOCUS_TIMMER").child("USER").child(sanitizedEmail).child("MOVIES_LIVE").setValue(jsonStr)

                    // Publish user stats to RTDB global movies leaderboard
                    val moviesCount = items.count { !it.isSeries && it.userStatus == com.example.data.MovieWatchStatus.COMPLETED }
                    val seriesCount = items.count { it.isSeries }
                    val totalEpsWatched = items.filter { it.isSeries }.sumOf { it.watchedEpisodesCalculated }
                    val completedMovieMinutes = items.filter { !it.isSeries && it.userStatus == com.example.data.MovieWatchStatus.COMPLETED }.sumOf { it.runtimeMinutes }
                    val seriesEpisodeMinutes = totalEpsWatched * 50
                    val totalHours = (completedMovieMinutes + seriesEpisodeMinutes) / 60f
                    val totalTitles = moviesCount + seriesCount

                    val myName = getMyNickname().ifEmpty { sanitizedEmail.split("@").firstOrNull() ?: "Me" }
                    val badge = when {
                        totalHours >= 100f -> "🥇 Binge King"
                        totalHours >= 50f -> "🥈 Cinephile"
                        totalHours >= 20f -> "🥉 Marathoner"
                        else -> "🍿 Movie Buff"
                    }

                    val statObj = com.example.data.GroupBingerStat(
                    ...
                    rtdb.getReference("FOCUS_TIMMER").child("MOVIES_LEADERBOARD").child(sanitizedEmail).setValue(statObj)
                }
            }
        } catch (e: Exception) {
            e.printStackTrace()
    ...
    fun startMoviesRtdbSync() {
        ...
        try {
            ...
            moviesRtdbListener?.let { rtdb.getReference("FOCUS_TIMMER").child("USER").child(sanitizedEmail).child("MOVIES_LIVE").removeEventListener(it) }
            leaderboardRtdbListener?.let { rtdb.getReference("FOCUS_TIMMER").child("MOVIES_LEADERBOARD").removeEventListener(it) }

            // Listener 1: Multi-device sync for movies watchlist
            val mListener = object : com.google.firebase.database.ValueEventListener {
                override fun onDataChange(snapshot: com.google.firebase.database.DataSnapshot) {
                    ...
                    if (!jsonStr.isNullOrEmpty()) {
                        try {
                            ...
                            val adapter = movieMoshi.adapter<List<com.example.data.MovieItem>>(listType)
                            val loaded = adapter.fromJson(jsonStr)
                            if (loaded != null && loaded != _movieTrackerItems.value) {
                                _movieTrackerItems.value = loaded
                                prefs.edit().putString("movie_tracker_json_data", jsonStr).apply()
                            }
                        } catch (e: Exception) {
            ...
            rtdb.getReference("FOCUS_TIMMER").child("USER").child(sanitizedEmail).child("MOVIES_LIVE").addValueEventListener(mListener)
            moviesRtdbListener = mListener

            // Listener 2: Real-user movies leaderboard
            val lbListener = object : com.google.firebase.database.ValueEventListener {
                override fun onDataChange(snapshot: com.google.firebase.database.DataSnapshot) {
                    if (!snapshot.exists()) {
                        _moviesLeaderboard.value = emptyList()
                        return
                    }
                    val list = mutableListOf<com.example.data.GroupBingerStat>()
                    for (child in snapshot.children) {
                        try {
                            val stat = child.getValue(com.example.data.GroupBingerStat::class.java)
                            if (stat != null) {
                                list.add(stat)
                            }
                        } catch (e: Exception) {
                            val userId = child.child("userId").getValue(String::class.java) ?: child.key ?: ""
                            val userName = child.child("userName").getValue(String::class.java) ?: "Binger"
                            val avatarUrl = child.child("avatarUrl").getValue(String::class.java) ?: ""
                            val hoursWatched = child.child("hoursWatched").getValue(Double::class.java)?.toFloat()
                                ?: child.child("hoursWatched").getValue(Long::class.java)?.toFloat() ?: 0f
                            val titlesCount = child.child("titlesCount").getValue(Long::class.java)?.toInt() ?: 0
                            val badge = child.child("badge").getValue(String::class.java) ?: "🍿 Movie Buff"

                            if (userId.isNotEmpty()) {
                                list.add(com.example.data.GroupBingerStat(userId, userName, avatarUrl, hoursWatched, titlesCount, badge))
                            }
                        }
                    }
                    _moviesLeaderboard.value = list.sortedByDescending { it.hoursWatched }
                }
                override fun onCancelled(error: com.google.firebase.database.DatabaseError) {}
            }
            rtdb.getReference("FOCUS_TIMMER").child("MOVIES_LEADERBOARD").addValueEventListener(lbListener)
            leaderboardRtdbListener = lbListener
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
    ...
    fun enrichMovieWithTmdbDetails(item: com.example.data.MovieItem, onComplete: ((com.example.data.MovieItem) -> Unit)? = null) {
        viewModelScope.launch(Dispatchers.IO) {
            val enriched = com.example.api.MovieSearchService.fetchTmdbRichDetails(item)
            if (enriched != item) {
                // Update in tracked items
                val currentTracked = _movieTrackerItems.value.toMutableList()
                val idx = currentTracked.indexOfFirst { it.id == item.id || (it.imdbId.isNotEmpty() && it.imdbId == item.imdbId) }
                if (idx >= 0) {
                    currentTracked[idx] = enriched
                    _movieTrackerItems.value = currentTracked
                    saveMovieTrackerData(currentTracked)
                }

                // Update in search results
                val currentSearch = _movieSearchResults.value.toMutableList()
                val sIdx = currentSearch.indexOfFirst { it.id == item.id || (it.imdbId.isNotEmpty() && it.imdbId == item.imdbId) }
                if (sIdx >= 0) {
                    currentSearch[sIdx] = enriched
                    _movieSearchResults.value = currentSearch
                }

                withContext(Dispatchers.Main) {
                    onComplete?.invoke(enriched)
                }
            }
        }
    }
...
data class TemporaryMediaViewData(
    ...
    val name: String,
    val mimeType: String,
    val mediaType: String, // "pdf", "video", "audio"
    val autoCompress: Boolean = false,
    val askCompressOrView: Boolean = false
)
