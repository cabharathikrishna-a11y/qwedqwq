...
object DynamicCommandManager {
    ...
    fun executeMidSessionCommand(
        ...
        val updatedTimeline = if (mappedAction == "START") {
            if (currentTimeline.isNotEmpty() && !isLastEnd) {
                currentTimeline + newEvent.copy(event = "RESUME")
            } else {
                listOf(newEvent)
            }
                ...
                if (task.isSuccessful) {
                    ...
                    kotlinx.coroutines.CoroutineScope(kotlinx.coroutines.Dispatchers.IO).launch {
                        try {
                            ...
                            if (statusStr.equals("Focusing", ignoreCase = true) || statusStr.equals("Running", ignoreCase = true) || statusStr.equals("Studying", ignoreCase = true)) {
                                PeerFocusFcmNotifier.notifyFriendsOnFocusStart(context, email, currentTask, currentTag)
                            } else {
                                PeerFocusFcmNotifier.notifyFriendsOnFocusStop(context, email)
                            }
                        } catch (e: Exception) {
    ...
    fun calibrateLocalState(
        ...
        if (hasLocalActive && isTargetIdle) {
            Log.d(TAG, "calibrateLocalState: Local device has an active/paused session while remote is idle/empty. Preserving local active timer.")
            return
        }
