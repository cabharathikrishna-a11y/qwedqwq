...
object AppBlockHelper {
    ...
    @Volatile private var cachedYtAllowSubscribedShorts = true
    @Volatile private var cachedYtBlockHomeFeed = true
    ...
    fun initializeCache(context: Context) {
        ...
        cachedYtAllowSubscribedShorts = ytPrefs.getBoolean("yt_allow_subscribed_shorts", true)
        cachedYtBlockHomeFeed = ytPrefs.getBoolean("yt_block_home_feed", true)
    ...
    fun isIgWebAppEnabled(context: Context): Boolean {
    ...
    @Volatile
    var isAntiGramWebAppOpen: Boolean = false

    fun isInstagramOpen(context: Context): Boolean {
        if (isAntiGramWebAppOpen) return true
        val currentFg = getForegroundPackageName(context)
        return currentFg == "com.instagram.android"
    }
    ...
    fun getForegroundPackageName(context: Context): String? {
        return try {
            ...
            val eventsStartTime = endTime - 10000L
    ...
    fun isInstagramReelOrNonMessageNotification(
        title: String,
        text: String,
        subText: String = "",
        ticker: String = ""
        ...
        val combined = "$title $text $subText $ticker".lowercase(java.util.Locale.ROOT)

        if (combined.isEmpty()) return false

        // Reel, video, post, suggested, reel shares, channel keywords
        val reelKeywords = listOf(
            "reel", "reels", "shared a reel", "sent a reel", "watch reel",
            "video", "shared a video", "posted a video", "watch video",
            "post", "shared a post", "sent a post", "posted a photo",
            "suggested", "suggested for you", "accounts you might like", "reels for you",
            "broadcast", "channel", "started a live", "is live", "live video",
            "instagram: reel", "instagram: reels"
        )

        for (keyword in reelKeywords) {
            if (combined.contains(keyword)) {
                return true // Reel/Video/Post/Suggestion -> Block
            }
        }

        // Direct chat message keywords
        val chatKeywords = listOf(
            "sent a message", "sent you a message", "messaged you", "message",
            "replied to your story", "replied to your message",
            "sent a photo", "sent a picture", "sent an image", "sent a video chat",
            "voice message", "audio message", "chat", "direct message"
        )

        for (keyword in chatKeywords) {
            if (combined.contains(keyword)) {
                return false // Chat message -> Allow
            }
        }

        // Direct person-to-person messages usually have title = sender name (not "Instagram") and non-empty text
        if (text.isNotEmpty() && !titleLower.contains("instagram")) {
            return false // Direct chat message -> Allow
        }

        // Non-chat notification -> Block
        return true
    }
    ...
    fun isYtWebAppEnabled(context: Context): Boolean {
        return context.getSharedPreferences("app_prefs", Context.MODE_PRIVATE).getBoolean("youtube_web_app_enabled", false)
    }

    fun setYtWebAppEnabled(context: Context, enabled: Boolean) {
    ...
    fun setInstagramReelsBlocked(context: Context, blocked: Boolean) {
        setIgReelsBlocked(context, blocked)
    }

    fun setInstagramStoriesBlocked(context: Context, blocked: Boolean) {
        setIgStoriesBlocked(context, blocked)
    }

    fun isIgMessagesBlocked(context: Context): Boolean {
        return context.getSharedPreferences("app_prefs", Context.MODE_PRIVATE).getBoolean("ig_messages_blocked", false)
    }
    ...
    fun isYtAllowSubscribedShorts(context: Context): Boolean {
        if (!isCacheInitialized) initializeCache(context)
        return cachedYtAllowSubscribedShorts
    }

    fun setYtAllowSubscribedShorts(context: Context, allowed: Boolean) {
        getYoutubePrefs(context).edit().putBoolean("yt_allow_subscribed_shorts", allowed).apply()
        cachedYtAllowSubscribedShorts = allowed
    }

    fun isYtBlockHomeFeed(context: Context): Boolean {
        if (!isCacheInitialized) initializeCache(context)
        return cachedYtBlockHomeFeed
    }

    fun setYtBlockHomeFeed(context: Context, blocked: Boolean) {
        getYoutubePrefs(context).edit().putBoolean("yt_block_home_feed", blocked).apply()
        cachedYtBlockHomeFeed = blocked
    }
