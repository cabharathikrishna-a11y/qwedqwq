...
class KeepAliveService : Service() {
    ...
    private fun hasActiveSession(): Boolean {
        ...
        val hasTimerProgress = totalSecsLeft < totalDurationSecs && totalSecsLeft > 0
        val isTimerPaused = !isTimerOn && hasTimerProgress
        ...
        return if (isTimerSelected) {
            isTimerOn || isTimerPaused
        } else {
            isStopwatchOn || isStopwatchPaused
        }
    }
    ...
    override fun onCreate() {
        ...
        // Periodically sync Total Focus Time widget while active without spamming IPC calls every second.
        // Stopwatch & Pomodoro widgets use Android native Chronometer, which ticks smoothly on the launcher automatically.
        serviceScope.launch {
            var lastUpdateMin = -1
            while (isActive) {
                val isRunning = (com.example.util.FocusTimerManager.isTimerRunning.value || com.example.util.FocusTimerManager.isStopwatchActive.value) && !com.example.util.FocusTimerManager.isPaused.value
                if (isRunning) {
        ...
        // Journal Photo Shower Widget auto-rotation loop based on configured interval in seconds
        serviceScope.launch {
            while (isActive) {
    ...
    private fun startFriendsFocusMonitoring() {
        ...
        if (isLoggedIn && currentUsername != null) {
            try {
                // Ensure PeerLiveSphereManager listens to all friends and FCM inbox in background service
                val emailToUse = if (currentUsername.contains("@")) currentUsername else prefs.getString("user_email", "") ?: ""
                val targetEmail = if (emailToUse.isNotBlank()) emailToUse else currentUsername
                if (targetEmail.isNotBlank()) {
                    com.example.api.PeerLiveSphereManager.startListeningToFriends(applicationContext, targetEmail)
                }

                // Setup real-time listener and profile configurations update (bypassed in offline-first mode)
