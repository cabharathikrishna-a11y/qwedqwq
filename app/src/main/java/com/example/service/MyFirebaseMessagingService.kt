...
class MyFirebaseMessagingService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(remoteMessage: RemoteMessage) {
        ...
        // 2. Check if payload is settings update signal, peer focus alert, or chat message
        if (data.isNotEmpty()) {
            val messageType = data["type"] ?: data["category"] ?: "text"

            if (messageType == "LIVE_DELTA_SYNC" || messageType == "LIVE_DELTA_SYNC_CHUNK") {
                Log.i(TAG, "Received FCM LIVE_DELTA_SYNC signal ($messageType). Delegating to LiveFcmSyncManager...")
                com.example.api.LiveFcmSyncManager.handleIncomingFcmSyncPayload(applicationContext, data)
                return
            }

            if (messageType == "PEER_FOCUS_START" || messageType == "FRIEND_FOCUS_START" || messageType == "peer_focus" || messageType == "friend_started_focusing") {
                val senderId = data["sender_id"] ?: data["friend_id"] ?: data["email"] ?: ""
                val myEmail = prefs.getString("user_email", "") ?: ""

                if (senderId.isNotBlank() && !senderId.equals(myEmail, ignoreCase = true)) {
                    val peerName = data["peer_name"] ?: data["display_name"] ?: data["sender"] ?: senderId.substringBefore("@")
                    val taskName = data["task_name"] ?: data["task"] ?: "Focus Session"
                    val tagName = data["tag_name"] ?: data["tag"] ?: "Study"

                    Log.i(TAG, "Received FCM peer focus start notification from $senderId ($peerName)")
                    com.example.api.PeerLiveSphereManager.showPeerFocusNotification(
            ...
            if (messageType == "PEER_FOCUS_STOP" || messageType == "FRIEND_FOCUS_STOP" || messageType == "friend_stopped_focusing") {
                val senderId = data["sender_id"] ?: data["friend_id"] ?: data["email"] ?: ""
                if (senderId.isNotBlank()) {
                    Log.i(TAG, "Received FCM peer focus stop notification from $senderId")
                    com.example.api.PeerLiveSphereManager.cancelPeerFocusNotification(
                        context = applicationContext,
                        friendId = senderId
                    )
                }
